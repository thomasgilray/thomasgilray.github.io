...
Programming Language Design is a tour of the surprisingly small set of ideas that nearly every programming language is assembled from. We will work in Python, Racket, Haskell, Rust, and C++, asking of each what it makes easy, what it refuses to allow, and what that refusal buys you. Rather than memorizing a language as a list of rules, we will build small interpreters for the ones we study, so that scope, binding, closures, evaluation order, recursion, mutable state, and types become things you have implemented rather than terms you have read. Alongside that comes just enough formal semantics (grammars and operational rules) to say precisely what a program *means*, and to see why two languages that look alike on the page can behave very differently when you run them. By the end you should be able to pick up an unfamiliar language quickly and reason about the tradeoffs its designers were making.
